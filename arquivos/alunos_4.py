...
def gravar_curso(nome_arquivo, numero_alvo, novo_curso):
    arquivo = open(nome_arquivo)
    novo_arquivo = open("temp.dat", "w")
    for linha in arquivo:
        (numero, nome, curso, n1, n2) = linha.split(",")
        if numero_alvo == int(numero):
            novo_arquivo.write("{},{},{},{},{}".format(numero, nome, novo_curso, n1, n2))
        else:
            novo_arquivo.write(linha)
    arquivo.close()
    novo_arquivo.close()
    remove(nome_arquivo)
    move("temp.dat",nome_arquivo)

def alterar_curso(nome_arquivo):
    print( "Informe o número do aluno: " )
    numero = int(input())
    print( "Informe o nome do novo curso: " )
    novo_curso = input()
    gravar_curso(nome_arquivo, numero, novo_curso)
